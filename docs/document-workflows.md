...
directory boundaries, and bounded dependency evidence. XLSX uses the same
`xlsx_sheet_evidence_v1` projection as editor argument generation. The selected
directory entry, capability, format, operation, and selection path are persisted
in the node's `OutcomeRefs`. The edit node can materialize only that entry. A
missing, stale, ambiguous, unsupported, or invalid decision blocks the Workflow.
The former inline secondary directory router has been removed; any other
multi-candidate scope must declare its own decision node. See the
