...
## 8. Implementation and Verification — 2026-09-10

The working tree implements policy `email-management-v4-source-events`. Durable activation and a resumable migration stop message/conversation summaries, relationship checks and presentation generation, including stale queued or leased work. Existing originals and historical artifacts remain. Migration schedules missing classification and unassigned events; it does not automatically regroup all historical assignments or rename existing events.

Classification consumes only the current original subject/body. A subject decision may escalate once to body; notification-looking subjects always do so to avoid overlooking concrete requests. Assignment retrieves bounded original evidence and explicit source/membership references, excluding generated summaries and event titles. Equivalent context writes, refreshes and renames do not invalidate classification. Each model stage has a 120-second timeout and each job generation at most two attempts. Evidence bounds are UTF-8 **bytes**, not the proposed character counts: current body 1,200, related/member body 500, at most ten candidates and two sampled members per candidate, with 8,000 total source-evidence bytes. Truncation and missing context are explicit; model request serialization also has a 48,000-byte ceiling.

The UI now offers category → event list → original timeline. Mixed events stay under interaction. Notification events also contain multiple mails. Purpose titles are separate from membership; missing naming cannot discard a proved event assignment, and source-subject fallback is labeled. Users can rename an event or move a mail into an existing/new event. Entity-version checks and idempotent commands update membership, candidate indexes and both event aggregates atomically while preserving provider IDs, RFC headers and viewing receipts. Late model results cannot overwrite a correction. Event title/source searches and category counts use the same event scope.

History projection is read-only and nonblocking, with pending/partial/paused/failed states. Durable worker scheduling pages known threads, reconciles already captured sources and deduplicates Sent/read mail; each history job drains at most four pages before continuation. Explicit sync can retry exhausted known history without making periodic polling revive terminal failures. Gmail has a tested direct native-thread route when the loaded list lacks that thread. QQ/Outlook have no proved equivalent independent lookup yet, and Gmail rendered membership does not prove full provider history. Missing sources remain visible as incomplete, not falsely complete.

Verification includes full Go tests/build/vet; focused race tests with real isolated PostgreSQL; three-backend membership, stale publication, restart and 100-refresh convergence tests; WebChat 125 tests, production build and 669 i18n keys; Controller 235 tests; and bilingual document checks. These are isolated tests, not a 30-minute live idle soak, production capacity qualification or real-provider end-to-end acceptance. No deployment, receiving-switch change or real email send occurred.

Synthetic model evidence is retained in [qualification metadata](evaluation/email-source-events-v4/qualification-metadata.json), [failed baseline](evaluation/email-source-events-v4/baseline.json), [first fresh holdout](evaluation/email-source-events-v4/fresh-holdout.json) and [final fresh holdout](evaluation/email-source-events-v4/final-fresh-holdout.json). After general prompt/occurrence-identity fixes, the final twelve newly frozen cases yielded 6/6 expected classification decisions and 6/6 event decisions (four automatic assignments, two expected pending). Earlier failures remain retained; the intermediate recheck is in-sample. The unpinned model alias and tiny corpus do **not** establish the proposed accuracy/error thresholds. All six final classifications needed body escalation, so subject-first efficiency is unproved. Verification-related appends require a shared explicit non-secret occurrence identifier; otherwise they remain pending, trading automatic coverage for fewer unsupported merges.

## 9. Remote Deployment — 2026-09-10

Owner-requested `npm run start:remote` deployed Gateway `41bcf429e80e` and WebChat `845d1ae206ac`. An initial Go dependency TLS transfer failure stopped before service replacement; retry succeeded without weakening TLS or changing dependencies. All five application services are running, Gateway readiness reports external/PostgreSQL, and WebChat returns HTTP 200. The host Browser Controller was restarted to refresh native script code; smoke passes and browser PID 1862115 remained unchanged with Bridge 1.0.22. Fast, Embedding, Guard and OCR model inventories return 200; inventory reachability alone is not inference qualification.

The persisted v4 email policy is active for both recorded owners. One historical migration remains in the job phase while the other completed; policy fences already prevent obsolete analysis publication during migration. Receiving states remain one active and two paused. No explicit Send was performed. Gateway logs also report WeChat context-sync session timeouts; remote email deployment does not resolve that separate session issue. The semantic and provider-completeness limits in section 8 still apply.
