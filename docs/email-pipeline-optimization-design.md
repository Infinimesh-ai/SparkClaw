...
This supersedes the future requirements in [classification v3](email-management-classification-v3.md) that excluded summaries and capture scripts and presented complete bodies by default. Historical implementation/deployment records remain valid. Keep notification/interaction routing and event-based conversations.
...
| Intake | [Stage 2](email-management-stage-2-intake.md) has recent/unread lanes, interval continuation, overlap, durable page acknowledgement and recovery | Reuse these semantics; unread priority must not re-download committed originals |
...
Show initial scope and coverage in settings. Retain the existing activation boundary, historical unread discovery and bounded history for encountered threads unless the user selects an earlier starting point. An earlier range becomes one resumable backfill that includes already-read mail within its selected folders. This request does not automatically authorize an unlimited whole-mailbox historical crawl.

Default scope is qualified ordinary inbound folders/labels and available Sent members of encountered threads. Exclude drafts, Spam/Trash and unqualified areas explicitly. Full history and unrelated outbound-event import are separate scope extensions. Prioritize fresh arrivals over historical backfill and show capture/analysis progress separately.
...
Persist state per owner/provider/mailbox/folder-or-lane/scope version: completed boundary, fixed current upper bound, continuation, stable original identities/versions and receipts, retry items/gaps, and separate initial-backfill progress.

1. Verify account and acquire the mailbox's single capture lease; fix the scan upper bound.
2. Resume unfinished intervals first. For new scans use the committed boundary plus overlap, initially the existing 24 hours, calibrated by provider. Use verified receipt/change ordering, never sender RFC Date.
