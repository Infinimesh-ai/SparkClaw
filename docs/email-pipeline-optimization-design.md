...
Status: 2026-09-10, implementation in `codex/email-pipeline-optimization`. Deployment/first-enable boundaries, removal of automatic unread scans, login recovery alerts, source-only summaries and collapsed local body loading are implemented and tested. **The three managed readers are implemented and installed in the dedicated browser; full-provider qualification is still in progress. Gateway/WebChat changes are not deployed.** The remaining sections describe the target behavior; they are not blanket acceptance claims.
...
- Gmail: native next-page UI and terminal state agree with HTTP observations. Network replay uses the response's opaque cursor as well as the page index; receipt-time filtering admits read messages and preserves inbound siblings of alternate-ID sent replies. Terminal certification rechecks scanned page fingerprints, excluding mutable read flags. A real full interval returned 58 then 11 inbound messages, with advancement only on the second page after prefix verification. A real empty interval returns zero candidates with a qualified terminal boundary: the observed null-list envelope also requires the matching native search URL and visible empty-result evidence.
- Outlook consumer: the early Worker bridge and native folder metadata bootstrap supply observed ItemRows/ItemExport operations. Complete FindFolders evidence, ordinary-folder ancestry, hidden flags and the unfiltered None view are required for broader coverage. Inbox-only or unknown hierarchy evidence cannot advance the whole interval. A real full folder scan returned two Inbox members followed by an empty Archive page, and advanced only after the final page and prefix verification.
- Both Gmail and Outlook have captured two real originals and replayed verified local receipts without downloading them again. Outlook has exercised native bootstrap followed by direct fetch-to-Blob capture. Gmail also exercised one exact-target native retry after a direct-download timeout; its observed final attachment origin is mail-attachment.googleusercontent.com. Gmail now passes native→network capture, byte-for-byte equality for the same original and receipt replay without download. Its task-owned Blob link uses a trusted browser click after slow inspections. Outlook also passes native→network capture and byte-for-byte equality for the same original; both providers pass their observed full-scope terminal scans. Consumer Outlook is qualified for this observed hierarchy; the Microsoft 365 Worker path remains unqualified and uses native fallback. QQ's latest standard login probe returned email_login_required; live QQ continuation awaits reauthentication. The QQ Reader retains observed-list/original support and the existing native network-observation fallback; general replay pagination and pinned/grouped coverage are not certified.
