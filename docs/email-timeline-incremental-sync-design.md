...
Normal polls and Refresh no longer request or claim legacy capture, thread-sync, mark-read or source-recovery jobs. A bounded, resumable and idempotent timeline policy retires persisted legacy queued/running/failed jobs while retaining prior error/attempt audit and persistent sync warnings. Late old leases cannot publish sources or complete into new parse work. Discover, parse and semantic jobs remain intact. QQ's newly qualified mark-read is a post-commit effect inside the active timeline round, not a revived per-message job: Store source commit happens first, a fresh provider list must confirm the exact target as read, and uncertainty leaves the durable source intact with remote state `unknown`. Memory, File and real isolated PostgreSQL tests cover this migration.
