...
The remaining `ALIAS` evidence records mutable alias escape for Schedule,
...
- 10 File mutation paths call `persist()`; `persist()` executes
  `_ = s.persistSnapshot()`. A caller can receive success after a failed
  snapshot, and later success may persist tentative state.
- PostgreSQL source contains 10 explicit discarded `Exec` results. The generic
...
- Three repository record families expose mutable aliases from Memory and the
