...
DeliveryRecord, BrowserState, Memory, and Evaluation records in both Memory and
the live File decorator. S0 does not repair those production contracts.
Approval and Audit now have positive isolation assertions. Each remaining owning
repository wave must replace its defect-evidence cell with an isolation
assertion.
