...
Conversation, Run, Schedule, Connector, PassiveNotification,
DeliveryRecord, BrowserState, and Memory records in both Memory and
the live File decorator. S0 does not repair those production contracts.
Approval, Audit, and Evaluation now have positive isolation assertions. Each
remaining owning repository wave must replace its defect-evidence cell with an
isolation assertion.
...
today 41 legacy File call sites discard the result. `P Exec` is the autocommit
submission point and `P Commit` is the transaction submission point; today 27
