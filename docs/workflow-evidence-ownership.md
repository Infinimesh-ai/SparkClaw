...
| Generic external MCP and coding agents | Configured endpoint identity, credential scope snapshot, catalog revision, namespaced entry ID, remote object ID/version, mutation class | Bounded eligible operation/object descriptions and untrusted returned content |
| LocalMind tasks | Configured endpoint identity, fixed contract revision, task ID, state version, current-message request, and internally generated idempotency key | The four explicit r1 Workflows expose only the bound local task operation; read/write status polling advances from validated outcome refs while results remain bounded untrusted data |
