#!/usr/bin/env python3
"""Offline provider tooling for native SparkClaw -> IMMS E2 reranker evidence v3."""
...
DEFAULT_CONTRACT_ROOT = (
    ...
    / "contracts"
    / "SparkClaw--IMMS"
    / "e2-reranker-evidence-v3"
)
ACCEPTED_MACHINE_COMMIT = "84be85643affd73cb8a8cc8c28212a697be713e7"
CONFORMANCE_MANIFEST_SHA256 = (
    "64cf1bbcefbc9fec1ae238d382cf69ba28c1d2f5c545bb041f3b9f19725eae51"
)
CONFORMANCE_MANIFEST_SIZE = 15492

MANIFEST_HEADER = "X-SparkClaw-Evidence-Manifest-SHA256"
DEPLOYMENT_HEADER = "X-SparkClaw-Deployment-Revision"
REQUEST_ID_HEADER = "X-Request-Id"
REQUEST_ID = "imms-e2-reranker-v3-capability-smoke-01"
RESPONSE_ID = "score-imms-e2-reranker-v3-capability-smoke-01"
...
def _validate_startup_argv(manifest: dict[str, Any]) -> None:
    ...
    if flags.get("--no-enable-log-requests") is not True:
    ...
    resource = serving["gpu_memory_utilization"]
    decimal = resource["decimal"]
    value = float(Decimal(decimal))
    if not 0 < value < 1 or struct.pack(">d", value).hex() != resource["float64_bits"]:
        fail(stage, "gpu_memory_utilization_invalid", "decimal/binary64 resource binding differs")
    if flags["--gpu-memory-utilization"] != decimal:
        fail(stage, "startup_argv_resource_mismatch", "resource argv differs")
...
class V3ContractBundle:
    ...
    def load(
        ...
        *,
        ...
        stage = "conformance_manifest"
        root = root.resolve()
        conformance_dir = root / "conformance" / "v3"
        ...
        if (
            ...
            fail(
                ...
                "central root manifest differs from accepted decision 0030 pin",
            )
    ...
    def validate_receipt(
        ...
        *,
        ...
        for flight_name in ("preflight", "postflight"):
            ...
            if any(metrics[key] > 2**53 - 1 for key in ("prefix_cache_hits_total", "prefix_cache_queries_total")):
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "check-contract":
            print(
                "PASS accepted native E2 reranker evidence v3: "
