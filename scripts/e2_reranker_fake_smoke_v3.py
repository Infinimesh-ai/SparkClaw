#!/usr/bin/env python3
"""Synthetic loopback-only native rerank client for E2 evidence v3."""
...
try:
    ...
    from e2_reranker_evidence_v3 import (  # type: ignore[no-redef]
        DEFAULT_CONTRACT_ROOT,
        DEPLOYMENT_HEADER,
        MANIFEST_HEADER,
        OBSERVATION_ORDER,
...
class SyntheticNativeRerankClient:
    ...
    def _metrics_projection(raw: bytes) -> dict[str, Any]:
        ...
        return {
            "configuration_source": "reviewed_deployment_configuration",
            "metric_labels": {"engine": "0", "model_name": "sparkclaw-reranker"},
    ...
    def run(self, manifest: dict[str, Any], manifest_raw: bytes) -> bytes:
        ...
        return self._collect(manifest, manifest_raw, allow_synthetic=True)

    def _collect(self, manifest: dict[str, Any], manifest_raw: bytes, *, allow_synthetic: bool) -> bytes:
        ...
        self.bundle.validate_manifest(manifest, allow_synthetic=allow_synthetic)
        ...
        receipt = {
            ...
            "artifact_class": "synthetic_fixture" if allow_synthetic else "live_evidence",
        ...
        return produce_receipt(self.bundle, manifest, receipt, allow_synthetic=allow_synthetic)
