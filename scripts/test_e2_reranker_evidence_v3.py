...
from scripts import e2_reranker_evidence_v3 as evidence
from scripts import e2_reranker_fake_smoke_v3 as smoke
...
class E2NativeRerankerEvidenceV3Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.bundle = evidence.V3ContractBundle.load()
    ...
    def test_root_pin_precedes_manifest_decode(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            path = root / "conformance/v3/manifest.json"
            path.parent.mkdir(parents=True)
            path.write_bytes(b"{not-json")
            with self.assertRaises(evidence.EvidenceFailure) as raised:
                evidence.V3ContractBundle.load(root)
    ...
    def test_root_inventory_rejects_unpinned_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(evidence.EvidenceFailure) as raised:
                evidence.V3ContractBundle.load(copied)
    ...
    def test_startup_argv_requires_exact_flags_values_and_order(self) -> None:
        ...
        manifest = copy.deepcopy(self.artifacts["deployment_manifest"])
        argv = manifest["serving"]["startup_argv"]
        i, j = argv.index("--enforce-eager"), argv.index("--no-enable-log-requests")
