...
func TestRuntimeAnswersFileReadWithLocalContent(t *testing.T) {
	...
	if !hasModelCallOperation(st.ListModelCalls(session.ID, result.Run.ID), "workflow_final_answer", "fast") {
...
func TestVisibleToolDefinitionsExposeDocxStructuredToolsForDocumentSkill(t *testing.T) {
	...
	names := visibleToolNames(defs)
	for _, want := range []string{"files.read", "docx.replace_paragraph", "docx.insert_paragraph", "docx.delete_paragraph", "docx.set_text_style", "pptx.add_slide", "pptx.update_slide", "pptx.duplicate_slide", "pptx.delete_slide", "xlsx.update_cell", "xlsx.insert_row", "xlsx.delete_row", "xlsx.update_row", "xlsx.append_row"} {
		if !slicesContainsString(names, want) {
