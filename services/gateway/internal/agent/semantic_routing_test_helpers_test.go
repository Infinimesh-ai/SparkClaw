...
func mustRouteIntentOutput(t *testing.T, runtime Runtime, sessionID, content string, resources []app.MessagePart, sourceKind app.MessageSourceKind) IntentRoutingOutput {
	...
	output, err := runtime.routeIntentWithRequest(t.Context(), sessionID, app.NewID("run_route_test"), content, resources, nil, sourceKind)
	if err != nil {
		t.Fatal(err)
	}
	return output
...
func (runtime Runtime) routeIntentOutputForTest(sessionID, runID, content string) (IntentRoutingOutput, error) {
	...
	return runtime.routeIntentWithRequest(context.Background(), sessionID, runID, content, nil, nil, app.MessageSourceWeb)
}
