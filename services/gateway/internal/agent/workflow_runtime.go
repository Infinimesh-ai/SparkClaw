...
func workflowArgumentAllowed(binding app.ArgumentBinding, node app.WorkflowNode, intent app.IntentEnvelope, route app.RouteDecision, state app.WorkflowNodeState, args map[string]any) bool {
	...
	for _, candidate := range workflowArgumentCandidates(binding, node, intent, route, state) {
...
func (r Runtime) bindWorkflowToolArguments(runID string, plan toolPlan) map[string]any {
	...
	for _, binding := range node.ArgumentBindings {
		...
		candidates := workflowArgumentCandidates(binding, node, run.Workflow.Intent, run.Workflow.Route, state)
		unique := ""
		ambiguous := false
...
func workflowArgumentCandidates(binding app.ArgumentBinding, node app.WorkflowNode, intent app.IntentEnvelope, route app.RouteDecision, state app.WorkflowNodeState) []string {
	allowed := []string{}
