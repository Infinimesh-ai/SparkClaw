...
func TestGatewayDeliversAllPartsThroughRegisteredProvider(t *testing.T) {
	...
	request := app.DeliveryRequest{SchemaVersion: app.DeliveryRequestSchemaVersion, ID: "del_fake", IdempotencyKey: "once", Target: app.EndpointID(binding.ID), Content: content, CreatedAt: time.Now().UTC()}
...
func TestProviderPreflightRejectsWholePayloadBeforeSend(t *testing.T) {
	...
	request := app.DeliveryRequest{
		...
		ID:            "del_unsupported", IdempotencyKey: "once", Target: app.EndpointID(binding.ID), CreatedAt: time.Now().UTC(),
		Content: app.MessageContent{Parts: []app.MessagePart{
...
func TestWorkflowAndExplicitSendShareDeliveryRequest(t *testing.T) {
	...
	result := app.WorkflowResult{SchemaVersion: app.WorkflowResultSchemaVersion, ID: "result_1", Content: content, ReturnRoute: route}
	...
	explicitRequest, deliver, err := RequestForMessage(t.Context(), "send_1", content, route, routes)
	if err != nil || !deliver {
