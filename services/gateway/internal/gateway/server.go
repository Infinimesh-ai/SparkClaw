...
func (s *Server) routes() {
	...
	s.mux.HandleFunc("DELETE /api/email/conversations/{conversation}", s.deleteEmailConversation)
	...
	s.mux.HandleFunc("GET /api/email/sync-warnings", s.listEmailSyncWarnings)
	s.mux.HandleFunc("POST /api/email/sync-warnings/{warning}/acknowledge", s.acknowledgeEmailSyncWarning)
	...
	s.mux.HandleFunc("GET /api/email/messages/{mail}/render-preview", s.getEmailMessageRenderPreview)
	s.mux.HandleFunc("POST /api/email/source/cleanup", s.cleanupEmailSource)
