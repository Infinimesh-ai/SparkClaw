...
var artifactMetadataRepositoryMethodSignatures = map[string]struct {
	parameters int
	results    int
}{
...
func TestArtifactMetadataRepositoryMigrationSourceGuard(t *testing.T) {
	...
	for _, declaration := range storeSource.Decls {
		...
		for _, specification := range general.Specs {
			...
			case "ArtifactMetadataRepository":
				repositoryMethods = len(interfaceType.Methods.List)
				for _, field := range interfaceType.Methods.List {
					if len(field.Names) != 1 {
						t.Errorf("ArtifactMetadataRepository contains an embedded or unnamed method")
						continue
					}
					expected, exists := artifactMetadataRepositoryMethodSignatures[field.Names[0].Name]
					...
					if !exists || !functionOK || countFieldList(function.Params) != expected.parameters ||
						...
						t.Errorf("ArtifactMetadataRepository method %s has an unexpected signature", field.Names[0].Name)
					}
				}
			case "Store":
				for _, field := range interfaceType.Methods.List {
					if len(field.Names) == 0 {
						if identifier, ok := field.Type.(*ast.Ident); ok && identifier.Name == "ArtifactMetadataRepository" {
							embeddedRepositories++
						}
						continue
					}
					if _, artifact := artifactMetadataRepositoryMethodSignatures[field.Names[0].Name]; artifact {
						repeatedMethods++
					}
				}
			}
	...
	err = filepath.WalkDir("../..", func(path string, entry fs.DirEntry, walkErr error) error {
		...
		for _, declaration := range parsed.Decls {
			...
			expected, artifact := artifactMetadataRepositoryMethodSignatures[function.Name.Name]
			if !artifact {
				continue
			}
			...
			if countFieldList(function.Type.Params) != expected.parameters || countFieldList(function.Type.Results) != expected.results ||
				...
				t.Errorf("%s has a legacy ArtifactMetadata signature in %s", function.Name.Name, path)
			}
		}
		ast.Inspect(parsed, func(node ast.Node) bool {
			switch current := node.(type) {
			case *ast.ExprStmt:
				if name := artifactMetadataRepositoryCallName(current.X); name != "" {
					ignoredCalls = append(ignoredCalls, path+":"+name)
				}
			case *ast.DeferStmt:
				if name := artifactMetadataRepositoryCallName(current.Call); name != "" {
					ignoredCalls = append(ignoredCalls, path+":"+name)
				}
			case *ast.GoStmt:
				if name := artifactMetadataRepositoryCallName(current.Call); name != "" {
			...
			case *ast.AssignStmt:
				for _, expression := range current.Rhs {
					name := artifactMetadataRepositoryCallName(expression)
			...
			case *ast.CallExpr:
				name := artifactMetadataRepositoryCallName(current)
	...
	for method := range artifactMetadataRepositoryMethodSignatures {
	...
	if len(ignoredCalls) != 0 {
		t.Errorf("ArtifactMetadata repository errors are ignored at %v", ignoredCalls)
	}
	if len(backgroundCalls) != 0 {
		t.Errorf("migrated ArtifactMetadata calls use context.Background at %v", backgroundCalls)
	}
}

func artifactMetadataRepositoryCallName(expression ast.Expr) string {
	...
	if _, artifact := artifactMetadataRepositoryMethodSignatures[selector.Sel.Name]; artifact {
