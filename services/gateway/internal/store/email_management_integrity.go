...
func validateEmailRecord(r EmailRecord) error {
	if r.Owner == "" || r.Owner != normalizeConnectorOwner(r.Owner) || r.ID == "" || strings.ContainsAny(r.ID, "\x00\r\n") || !containsEmail([]string{"draft", "send_snapshot", "mailbox", "mail", "capture", "representation", "context", "conversation", "decision", "concern", "concern_link", "target", "dependency", "reference", "refresh", "job", "thread", "sync", "view", "command", "counter", "summary", "sender_rule", "presentation"}, r.Kind) {
