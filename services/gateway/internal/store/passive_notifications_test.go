...
func TestMemoryStorePassiveNotificationIdempotentReingestionAtScale(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		...
		existing, inserted, err := st.CreatePassiveNotification(notification)
	...
	if _, _, err := st.CreatePassiveNotification(tampered); !errors.Is(err, ErrPassiveNotificationConflict) {
		t.Fatalf("tampered replay error = %v", err)
	}
	if got := st.CountUnreadPassiveNotifications(app.DefaultOwnerID); got != total {
...
func TestPrunePassiveNotificationsRetentionSweep(t *testing.T) {
	...
	if removed := st.PrunePassiveNotifications(time.Time{}, 0); removed != 0 {
		t.Fatalf("disabled prune removed %d", removed)
	}
	if removed := st.PrunePassiveNotifications(now.AddDate(0, 0, -7), 0); removed != 1 {
	...
	if _, inserted, err := st.CreatePassiveNotification(old); err != nil || !inserted {
...
func TestPrunePassiveNotificationsCapEvictsReadOldestFirst(t *testing.T) {
	...
	for _, id := range []string{"read-old", "read-new"} {
		if _, err := st.MarkPassiveNotificationRead(app.DefaultOwnerID, id, time.Time{}); err != nil {
			t.Fatal(err)
		}
	}

	if removed := st.PrunePassiveNotifications(time.Time{}, 2); removed != 3 {
	...
	if removed := st.PrunePassiveNotifications(time.Time{}, 2); removed != 0 {
...
func TestPassiveNotificationRevisionSignalsInboxChanges(t *testing.T) {
	...
	if got := st.PassiveNotificationRevision(owner); got != 0 {
	...
	afterCreate := st.PassiveNotificationRevision(owner)
	if afterCreate == 0 {
	...
	if got := st.PassiveNotificationRevision(owner); got != afterCreate {
		t.Fatalf("idempotent replay changed revision %d -> %d", afterCreate, got)
	}
	if _, err := st.MarkPassiveNotificationRead(owner, notification.ID, time.Time{}); err != nil {
		t.Fatal(err)
	}
	afterRead := st.PassiveNotificationRevision(owner)
	...
	if got := st.PassiveNotificationRevision(owner); got != afterRead {
		t.Fatalf("repeated mark-read changed revision %d -> %d", afterRead, got)
	}
	if _, err := st.MarkAllPassiveNotificationsRead(owner, time.Time{}); err != nil {
		t.Fatal(err)
	}
	if got := st.PassiveNotificationRevision(owner); got != afterRead {
		t.Fatalf("no-op mark-all changed revision %d -> %d", afterRead, got)
	}
	if removed := st.PrunePassiveNotifications(time.Now().UTC().Add(time.Minute), 0); removed != 1 {
		t.Fatalf("prune removed %d", removed)
	}
	if got := st.PassiveNotificationRevision(owner); got == afterRead {
		t.Fatal("prune did not bump the revision")
	}
	if got := st.PassiveNotificationRevision("other-owner"); got != 0 {
...
func TestFileStoreSnapshotRebuildsPassiveNotificationIndex(t *testing.T) {
	...
	if _, inserted, err := reloaded.CreatePassiveNotification(notification); err != nil || inserted {
		t.Fatalf("reload dedup = %v, %v", inserted, err)
	}
	tampered := notification
	tampered.Fingerprint = "fingerprint-tampered"
	if _, _, err := reloaded.CreatePassiveNotification(tampered); !errors.Is(err, ErrPassiveNotificationConflict) {
	...
	if _, inserted, err := reloaded.CreatePassiveNotification(second); err != nil || !inserted {
		t.Fatalf("create second = %v, %v", inserted, err)
	}
	if removed := reloaded.PrunePassiveNotifications(time.Time{}, 1); removed != 1 {
		t.Fatalf("prune removed %d", removed)
	}
	...
	items := final.ListPassiveNotifications(app.DefaultOwnerID, "", 10)
...
func TestFileStoreLoadsLegacySnapshotWithoutPassiveNotificationFields(t *testing.T) {
	...
	if got := st.PassiveNotificationRevision(app.DefaultOwnerID); got == 0 {
