...
(async () => {
  ...
  const workbook = new ExcelJS.Workbook();
	  ...
	  sheet.addRow(["Name", "Amount", "Double", "Note", "Link"]);
	  sheet.addRow(["Alpha", 42, { formula: "B2*2", result: 84 }, "Keep me", { text: "Docs", hyperlink: "https://example.com/docs" }]);
	  sheet.getCell("B2").numFmt = "0.00";
	  sheet.getCell("B2").font = { bold: true };
	  sheet.getCell("D2").note = "Retained comment";
