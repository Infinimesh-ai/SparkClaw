	...
	return []app.ToolDefinition{
		...
		},
		{
			...
			InputSchema: schema("object", []string{"url"}, map[string]any{
				...
				"auth_strategy":           stringSchema(),
				"login_handoff_completed": booleanSchema(),
				"persist_browser_auth":    booleanSchema(),
				"save_browser_auth":       booleanSchema(),
			}),
			OutputSchema: objectSchema([]string{"url", "status_code", "content_type", "title", "text", "bytes", "truncated", "untrusted", "untrusted_external_content", "warning"}, map[string]any{
				...
				"browser_auth_record_id":         stringSchema(),
				"browser_profile_id":             stringSchema(),
				"owner_id":                       stringSchema(),
				"account_hint":                   stringSchema(),
				...
				"browser_auth_restore_attempted": booleanSchema(),
				"browser_auth_restore_succeeded": booleanSchema(),
				"browser_auth_restore_error":     stringSchema(),
				"browser_auth_export_error":      stringSchema(),
				"browser_auth_record_saved":      booleanSchema(),
			}),
			Risk:             app.RiskRead,
			RequiresApproval: false,
			Idempotent:       true,
