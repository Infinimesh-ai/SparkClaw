...
test('listener precedes click, original download is returned, staging is removed',async()=>{
 ...
 const page={url:()=>url,locator:()=>({waitFor:async()=>{},evaluate:async()=>assert.ok(listening,'must listen before click')}),evaluate:async()=>true,waitForEvent:()=>{listening=true;return Promise.resolve(download)}};
 try{
...
test('filtered selection fails before clicking or downloading',async()=>{
 const dir=await fs.mkdtemp(path.join(os.tmpdir(),'ai-chat-test-'));
 ...
 try{await assert.rejects(captureAIChat({provider:'chatgpt',url,outputDir:dir,runCode:code=>new Function(`return (${code})`)()(page)}),/selection_or_loading/);assert.equal(clicked,false)}finally{await fs.rm(dir,{recursive:true,force:true})}
});
...
test('native Chromium download plumbing (synthetic page, not platform qualification)', {skip:!process.env.SPARKCLAW_TEST_CHROMIUM}, async()=>{
 ...
 try{
  ...
  await page.route('**/*',r=>r.fulfill({contentType:'text/html',body:`<div id="export-controls-container"><button id="export-json-btn">↓ JSON</button></div><div id="export-outline-container"><input id="outline-select-all" type="checkbox" checked><input id="outline-search-input" value=""></div>`}));
  await page.goto(url);
  await page.evaluate(doc=>{document.querySelector('#export-json-btn').onclick=()=>{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(doc)],{type:'application/json'}));a.download='conversation.json';a.click();};},doc);
  for(let i=0;i<3;i++){
