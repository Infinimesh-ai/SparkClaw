...
Catalog revision `2026-07-20.v5` 有五个生产叶子：`browser.internet_search`、`browser.weather`、`browser.automation`、`document.read` 和 `document.edit`。`WorkflowProfileRegistry.Resolve` 把前四个叶子映射到 revision 1，把 `document.edit` 映射到 revision 2，不再执行意图匹配。Dispatcher 持久化 `RouteDecision`、`ReturnRoute`、已校验 Plan Digest 与 Node State。
