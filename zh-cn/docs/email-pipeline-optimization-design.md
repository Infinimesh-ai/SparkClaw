...
状态：2026-09-14，在当前工作树实施。部署／首次启用时间边界、仅网络接口读取、按时间区间增量发现、登录恢复红点、仅依赖原文的摘要、正文折叠与本地按需加载已编码并通过测试。**三家托管 Reader 已生成并安装到专用浏览器；QQ 网络列表与原件采集已完成两次实机验收。Outlook 当前没有可观察的 Worker 列表模板，Gmail 当前没有可观察的 `/sync/u/<account>/i/bv` 列表请求，两家适配器等待重新观察当前提供方请求后再维护验收。** 下文描述目标行为，不代表全部验收完成。

## 当前读取契约（2026-09-14）

- 托管 Reader 是邮件读取的唯一来源。发现、列表分页、稳定邮件身份、原件字节和远端已读状态，都必须来自专用浏览器中已登录页面由 Reader 观察或重放的网络请求。
- 不保留 DOM 读取、菜单导出、页面点击或 native fallback。上游接口变化时，在专用浏览器重新观察真实请求，维护对应提供方适配器并重新验收；不增加宽泛兼容层。
- 只有一个 `recent_inbound` 发现通道，且每次请求都必须带 `interval_start`、`interval_end`。普通同步使用半开区间 `[last_completed_fetch_time, fixed_current_time)`；首次同步从持久化部署下界开始。邮件在其他设备已读仍然纳入，`read_state` 只作证据，不参与筛选。
- 只有列表分页完整、原件已持久化且 Gateway／Store 确认后，才能推进完成边界。采集与 mark-read 解耦；mark-read 是独立网络能力，失败不撤销成功采集，也不推进边界。
- 不存在 `unread` 发现通道和 `inbox_unread` coverage。分页不完整、未知行或网络错误必须保留缺口并阻止边界推进。
...
| 环节 | 已核对的现状 | 本次需要完善 |
|---|---|---|
| QQ 历史原型 | 旧独立脚本用于普通文件夹第一页及原件读取试验，已被受管 Reader 替代，2026-09-17 删除原型及专属测试 | 当前功能与验收见[时间线设计](email-timeline-incremental-sync-design.md)，本表缺口仅为历史记录 |
...
三个受管入口为 `qq-mail-reader.user.js`、`gmail-mail-reader.user.js`、`outlook-mail-reader.user.js`，共享结果结构和同步语义，各自维护站点网络请求解析。QQ 试验脚本只作历史参考，产品源码统一放入 `tools/browser-userscripts/`；不维护两份可独立变更的生产源码。

每个适配器提供：检查登录与实际账号、枚举时间区间及后续页、从网络响应识别线程单封成员、按稳定提供方 ID 获取原件、取消操作、返回覆盖与错误。具体命令字段是 SparkClaw 内部设计，不是已注册接口。
...
复用现有原件下载和附件提取；模型只读取主题与正文，不分析附件。采集成功不依赖远端 mark-read：mark-read 只有在对应网络请求已观察并验证时才单独执行；失败记录为独立能力错误，不影响原件入库和同步边界。网络读取产生的已读状态只作观测，不把本地查看摘要等同于远端已读。
...
它指**上次连续完整枚举且原件已持久化的区间终点**，不是上次点击按钮的时间、任务启动时间、最新一封的发件时间或摘要完成时间。普通同步使用半开区间 `[last_completed_fetch_time, fixed_current_time)`；上界在发现开始前固定，只有完整区间确认后才推进。
...
2. 优先续跑未完成区间；新一轮将 `interval_start` 设为上次完成边界（首次同步则为 `deployment_started_at`），并一次性固定 `interval_end` 为当前时间，枚举半开区间 `[interval_start, interval_end)`。使用提供方接收／变更次序发现候选，再按实际接收时间确认部署范围，不能用可回填的 RFC Date；部署前邮件仅因部署后移动／变更被再次发现，不应自动纳入。
...
摘要注明生成状态和语言。摘要任务开始时读取 WebChat 当前选择的语言，并把该语言随结果一起保存；之后切换语言只影响此后才读取偏好的任务，不翻译、不失效、也不重新生成已经发布的摘要。待生成、生成中、失败、更新中分别展示。摘要缺失时展示主题／发件人和状态，可直接查看正文；不用截取正文伪装成已生成摘要。事件变更期间可保留旧摘要，但标明更新中。引用邮件不存在或已失去访问权限时，不保留可误导用户的过期来源内容。
...
- 分类调用模型前，可用严格的原文规则识别验证码通知：必须同时出现明确的验证码／OTP 标记和邻近、包含数字的验证码形态 token。命中后直接发布带版本的 `notification/verification` 分类，并跳过该邮件的生成摘要和事件归属；普通数字、订单号／发票号或只有提示词的内容仍进入语义分析。
- 单封摘要的缓存键由原件内容版本和摘要策略版本确定；事件摘要的缓存键由成员 ID／原件版本集合和摘要策略版本确定。任务采样到的输出语言是不可变结果元数据，不是触发历史摘要失效的依赖。查看、分页、已读、同步心跳不改变键。
- 新增／移除事件成员或源内容改变才更新事件摘要。人工改分类但成员不变时，不重算单封摘要。
...
| 部署时间下界 | 部署前邮件不因未读而获取、部署后已读照常获取；重启／升级／晚绑定邮箱不重置下界；线程和区间扫描不越界补原件 |
...
- Gmail 和 Outlook 均有两封真实原件的网络获取及回执重放证据，重放不重复下载。此前通过原生界面学习请求的记录只作历史证据；当前 Reader 只走已观察的网络适配器，接口不匹配时显式失败，不保留原生回退。Microsoft 365 Worker 路径以及 QQ 通用重放分页、置顶／分组覆盖尚未认证。本次专用浏览器测速已两次完成 QQ；Outlook 因没有 `ItemRows`／Inbox `ConversationRows` Worker 模板而停止，Gmail 因没有 `/sync/u/<account>/i/bv` 列表请求而停止，失败耗时只作诊断，不作为服务商获取速度。
- 摘要依赖仅含原文与事件成员，100 次刷新回归确认不重复生成、不反向触发分类。新摘要任务读取持久化的 WebChat 语言偏好（`zh`／`en`）；切换语言不会改动已经发布的摘要。
- 含明确标记和邻近验证码形态 token 的验证码通知会在模型调用前由带版本的确定性规则分类。Store 只接受受约束的 `pattern` 结果，并且不再为该邮件安排摘要或事件归属模型任务；普通订单号／发票号仍走模型链路。
