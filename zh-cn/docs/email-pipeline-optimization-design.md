...
状态：2026-09-10，依据用户本轮要求整理的优化设计，**尚未实施、导入油猴或部署**。产品要求已经明确；下文的预算、数据结构和实施顺序为建议值，实施时根据实际接口与验证结果调整。
...
2. 首次整理完成后，每次沿上次已完成的获取时间线同步尚未获取的邮件，不重复全量获取与分析。
3. 恢复摘要，改善分类后的可读性。默认阅读摘要，正文由用户按需点击查看。首次整理允许承担较大工作量，日常仅处理新增及实际受影响的内容。

本设计替代[分类与事件对话 v3](email-management-classification-v3.md)中“不生成邮件／对话摘要”“正文默认完整展示”“采集脚本不在范围”的后续产品要求；该文档记录的历史实现和部署事实仍然保留。继续采用通知／交互分类和以事件为最小对话单位，不因恢复摘要改变事件归属规则。
...
| 增量采集 | [接收设计](email-management-stage-2-intake.md)已有近期／未读双通道、时间区间、重叠窗口、分页确认与恢复；默认空闲间隔 20 分钟、每页至多 50 封 | 网络适配器复用可靠进度语义；未读仅作优先级，不能重复下载已获取原件 |
...
首次建立账号独立的基线，在设置中展示“首次整理范围”与已覆盖时间。沿用当前激活时间后的邮件、历史未读和已遇到线程的有界往来补齐，不将本轮要求自动解释为无限全邮箱历史回溯。若用户选择更早起点，则作为一次可暂停、可续跑的历史整理任务，并纳入对应文件夹内已读邮件。

默认覆盖已验证的普通收件文件夹／标签和已遇到线程的可用已发送成员；草稿不作为正式邮件，垃圾／已删除及未验证范围明确排除。完整历史、独立已发送事件的全量导入等范围扩展须在设置和验收中单独说明。

首次历史整理低于新邮件同步优先级，采集与分析进度分开显示。原件可分批入库，摘要可渐进出现；不必等历史全部分析完成才使用邮箱。
...
2. 优先续跑未完成区间；新一轮从 `completed_through` 向前保留一个重叠窗口查询到固定上界。沿用现有 24 小时重叠作为初始值，按提供方实测调整；使用提供方接收／变更时间，不能用可回填的 RFC Date。
...
本轮只交付设计文档，没有编写或安装 Gmail／Outlook 脚本、启动同步、运行模型或变更生产邮件数据。
