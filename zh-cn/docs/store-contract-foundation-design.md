...
代表性的后端无关跨契约 harness 只有在配套完整的逐 repository
适用性/证据矩阵时才能跨 repository 共享证据。每个 repository 和维度都必须
引用精确的现有/新增测试，或以契约专属理由记录 `N/A`。后续 S2/S3 波次针对
已迁移方法集增加失败契约覆盖，而不是替换这些 S0 证据。
