...
> 状态：用户已于 2026-08-20 授权开始 S0；S0 实现审查仍为 pending。
> 本文档不授权 S1 或 runtime 变更。每份关联设计和每个实现结果都必须得到
> 独立且有记录的审查结论，才能开始下一个 Store 阶段。
...
| S0 实现 | 候选 SHA 待填写 | `pending` | 等待人工审查 S0 清单、manifest、测试和验收报告 | pending |
