...
剩余 `ALIAS` 证据记录 PassiveNotification 和 DeliveryRecord 值在
Memory 以及运行中的 File decorator 中存在可变别名逃逸。包括 Memory 在内的
已迁移 repository 已有正向隔离断言；其余每个负责的 repository wave 必须用
隔离断言替换相应 defect-evidence 单元格。
