...
还固定了这个已迁移边界的 12 个 context/error 签名、一次 Store 嵌入、三个后端
实现，以及生产调用的错误和 context 规则。

原 PassiveNotification 与 DeliveryRecord 的 `ALIAS` 缺陷证据已由正向的
Memory/File 隔离断言替换。其余 repository wave 接管可变记录契约时必须完成
相同替换。
...
`F rename` 是未来的持久 effect submission point；当前 10 个旧 File 调用点会
丢弃结果。`P Exec` 是 autocommit submission point，`P Commit` 是事务
submission point；当前 10 个 PostgreSQL `Exec` 结果被显式丢弃。未来必需的
...
| Delivery receive save | receive record 和 source/native key | receive audit | record + audit；M lock / F rename，带 rollback 或 unknown-outcome fence / P Commit | 唯一 ID 与 source/native 锁 | `FindMessageReceive`、`ReconcileMessageReceiveWrite` |
| Delivery send save | delivery record 和 owner/actor/idempotency key | send audit | record + audit；M lock / F rename，带 rollback 或 unknown-outcome fence / P Commit | 唯一 ID 与 owner/actor/key 锁；target 和 content digest 不可变 | idempotency lookup、`ReconcileMessageDeliveryWrite` |
| Delivery inbox update save | update 和 binding/external key | 无 | 单 update；M lock / F rename，带 rollback 或 unknown-outcome fence / P Commit | 唯一 ID 与 binding/external 锁 | `FindChannelInboxUpdate`、`ReconcileChannelInboxUpdateWrite` |
...
| Artifact `SaveArtifactObject` | object 加 URI-to-ID 派生索引 | saved audit + event | object + lifecycle/index；M lock / F rename / P transaction | ID overwrite 和索引替换 | `FindArtifactObjectByURI` |
...
- 6 条 File mutation 路径调用 `persist()`；`persist()` 执行
  `_ = s.persistSnapshot()`。snapshot 失败后 caller 仍可能收到成功，之后的
  成功操作还可能持久化暂存状态。
- PostgreSQL 源码包含 7 个显式丢弃的 `Exec` 结果。通用 `collectRows` 路径
  和六个具名 list/prune 函数不会传播 scan/iteration 完成失败；共有 8 个
...
- 2 个 repository 记录族会从 Memory 和运行中的 File decorator 暴露可变
