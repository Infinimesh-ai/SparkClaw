...
剩余 `ALIAS` 证据记录 Schedule、PassiveNotification 和 DeliveryRecord 值在
...
- 10 条 File mutation 路径调用 `persist()`；`persist()` 执行
  `_ = s.persistSnapshot()`。snapshot 失败后 caller 仍可能收到成功，之后的
  成功操作还可能持久化暂存状态。
- PostgreSQL 源码包含 10 个显式丢弃的 `Exec` 结果。通用 `collectRows` 路径
...
- 3 个 repository 记录族会从 Memory 和运行中的 File decorator 暴露可变
