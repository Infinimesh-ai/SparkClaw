...
- 每个自然语言 Owner Request 都先完成规范化与 Grounding，再由同一语义图上的 Embedding
  与 Fast/Tree 通道分类；Fusion 和有界 Reranking 最多选择一个经 Catalog 校验的叶子。
...
| `document.edit` r2 | 对一个受治理的 text、DOCX、XLSX、PPTX 或 PDF 执行一次受支持的有界编辑，并在同目录写入新副本。 | 先确定性校验路径和类型并结构化读取，再只物化一个匹配格式与操作的编辑工具。输出路径冻结为 `<name>-sparkclaw-edit.<ext>`。 | 输入必须通过与文档读取相同的 Workspace 校验，输出文件不能已经存在，且可逆写操作需要审批。不支持新建文档、通用文件删除、多文件编辑和未声明操作。 |
