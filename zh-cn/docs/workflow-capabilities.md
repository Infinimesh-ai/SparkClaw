...
| `document.edit` r4 | 对一个受治理的 text、DOCX、XLSX、PPTX 或 PDF 执行一次受支持的有界编辑，并写入一个或多个可追溯输出副本。 | 确定性路径/类型 preflight -> `confirm_document_target` -> `document_locate_evidence` -> 显式且有重试上限的 `select_edit_operation` 决策 -> `document_edit` 只物化已持久化的格式/operation entry。默认输出路径冻结为 `<name>-sparkclaw-edit.<ext>`。 | 多候选 operation 选择由 Deep 基于定位证据完成，单候选格式确定性选择；决策缺失或无效时显式 block。原 Fast 二次路由已删除，其他多候选 scope 同样必须声明决策节点。可逆写入需要审批，每个输出都关联 parent 和 activity。 |
