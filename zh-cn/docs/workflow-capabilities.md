...
|---|---|---|---|
| `conversation.answer` r3 | `answer` 变体回答问候、稳定常识和简单解释；`publish` 变体返回普通有序 image、audio 和 file part。入站 MCP 对话可以用相对路径、精确 basename 或不完整文件名描述指定已有 workspace 媒体。 | 始终先执行 `detect_response_media` 并冻结 `none`、`selected`、`clarify` 或 `blocked`，再由 `answer` 执行一次无工具模型回答或确定性发布冻结内容。精确 basename 匹配区分大小写；未命中或名称不完整时使用共享、有界且只匹配文件名的查询，并选择稳定 Top-1。结果可以组合回答文本与冻结媒体。 | 外部 MCP 只暴露 `sparkclaw.conversation.send`，不暴露 `files.search` 或 Catalog 叶子。查询不读取文件内容、不返回临时候选，也不接受绝对路径。answer 节点不能再次查询或替换资源，hash/size 变化会阻断整个 multipart 结果。非对话能力仍由普通路由选择独立 Profile。 |
