...
| `browser.email` r2 | 通过已配置的 QQ 邮箱、Outlook 或 Gmail 账户新发送一封纯文本邮件。人工读取入口返回来源清单；独立默认关闭的[管理服务](email-management-implementation.md)负责 Store 接收和分析。 | 确定性的显式/默认账户选择与新鲜登录准入绑定固定后台脚本。发送接受收件人、可选单行主题和正文。当前接收通过提供方网络响应枚举上次完整获取时间之后的区间，已读与未读均纳入，并将原件及范围内附件采集为工作区文件；读取无需模型参数。 | `email.send` 保留精确内容审批、新鲜校验、最多一次发送和未知结果处理。读取无发送权限；完整采集持久保存后才独立执行 mark-read。QQ 自动接收默认启用已通过实机验收的精确目标标已读，且仅在新提供方列表确认后记录 `read`；Gmail／Outlook 继续 fail-closed。`script_capture` 不等于 Store 已提交邮件或模型总结。限制和缺失材料均明确记录，见[来源设计](email-read-design.md)。回复、草稿管理与通用邮箱搜索仍不可用。 |
