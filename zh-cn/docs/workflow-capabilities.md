...
| `browser.interaction` r3 | 检查一个托管当前 tab、明确 HTTP(S) URL、注册站点或经 Info 识别的公网网站，执行最多三次独立验证、ref-bound click，并保持已验证的可视结果打开。 | 必要时先识别目标 -> automation r3 acquisition -> hidden `browser.assess_goal` -> 有界 `browser.click` -> settle -> fresh snapshot -> `browser.validate_transition` -> `browser.assess_goal`；有进展时可重复 action loop。完成时可加入一次 owner 明确请求的视觉检查，再执行 visible open/focus -> settle -> fresh snapshot -> 第二次 `browser.assess_goal`。 | 所有参数都绑定到持久化 session/page generation 和 fresh ref。stale evidence、重复状态、route divergence、transition failure 或第三次 progress 都会 fail closed。登录和人工验证进入持久化 owner handoff。type、select、upload/download、凭据、captcha/2FA、payment/purchase、form submit、任意脚本和 unsafe consequential action 不属于 r3。 |
...
| `browser.email` r1 | 通过已配置的 QQ 邮箱、Outlook 或 Gmail 浏览器账户，向一个收件人新发送一封纯文本邮件。 | 语义 Send 路由 -> 确定性选择请求中明确指定或默认的提供方 -> Workflow 创建前在 Headless Chromium 中执行配置层所有的新鲜登录探针 -> 单个 `email_send` 节点。模型只提供收件人、可选单行主题和正文；Runtime 在精确内容审批及一次 `email.send` 调用前恢复提供方、账户、Setting Version、Browser Generation、Probe/Send Script Revision、校验时间和 Invocation ID。 | 审批后再次校验提供方设置与 Headless Browser Generation。提供方脚本只创建并使用任务所有标签页，校验编写字段和“发送”控件，且最多尝试发送一次。未知结果是终态且不可重试。Headed Chromium 只用于人工登录。不支持读取、回复、草稿、附件、多收件人/账户、通用浏览器工具、容器 Chromium、Playwright 或浏览器回退。 |
